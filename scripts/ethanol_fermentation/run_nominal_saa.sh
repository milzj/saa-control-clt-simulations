#!/usr/bin/env bash
# Run the 'nominal_saa' study for ethanol_fermentation, timed, with stdout+stderr logged.
...
LOG="$LOGDIR/nominal_saa_$STAMP.log"
echo "[run] $EXAMPLE nominal_saa  ->  $LOG"
cd "$HERE"
# Time the solve and tee stdout+stderr (incl. the 'time' report) into the log.
{ time MPLBACKEND=Agg "$PY" nominal_saa.py "$@" ; } 2>&1 | tee "$LOG"
